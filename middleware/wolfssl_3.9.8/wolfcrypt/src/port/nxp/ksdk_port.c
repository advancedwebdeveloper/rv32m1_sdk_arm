...
static bool cau3_keyslot_is_loaded(const void *ctx)
{
    bool ret = false;
    for (int i = 0; i < 4; i++)
    {
...
int wc_AesSetKey(Aes* aes, const byte* userKey, word32 keylen, const byte* iv, int dir)
{
    ...
    cau3_detach_ctx_from_key_slot(aes);
...
void wc_AesEncryptDirect(Aes *aes, byte *out, const byte *in)
{
    ...
    if (!cau3_keyslot_is_loaded(aes))
    {
        CAU3_AES_SetKey(CAU3, &s_cau3Handle, key, keySize);
        cau3_attach_ctx_to_key_slot(aes, s_cau3Handle.keySlot);
    }
...
int wc_Des3_SetKey(Des3 *des, const byte *key, const byte *iv, int dir)
{
    ...
    cau3_attach_ctx_to_key_slot(des, s_cau3Handle.keySlot);
