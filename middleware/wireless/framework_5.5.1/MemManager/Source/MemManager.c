...
*
********************************************************************************** */
#if (defined MULTICORE_MEM_MANAGER) && ((MULTICORE_HOST) || (MULTICORE_BLACKBOX))
#if MULTICORE_HOST
memStatus_t MEM_BufferFreeOnMaster
#elif MULTICORE_BLACKBOX
...
#endif /* (defined MULTICORE_MEM_MANAGER) && ((MULTICORE_HOST) || (MULTICORE_BLACKBOX)) */
...
)
{
    ...
    if( ((uint8_t*)pHeader < (uint8_t*)memHeap) || ((uint8_t*)pHeader > ((uint8_t*)memHeap + sizeof(memHeap))) )
    {
#if (defined MULTICORE_MEM_MANAGER) && (MULTICORE_BLACKBOX)
        return MEM_BufferFreeOnMaster(buffer);
#elif (defined MULTICORE_MEM_MANAGER) && (MULTICORE_HOST)
...
uint32_t MEM_WriteReadTest(void)
{
  ...
  for(idx1=0; poolInfo[idx1].blockSize != 0; idx1++)
  {
    for(idx2=0; idx2 < poolInfo[idx1].poolSize; idx2++)
    {
      ...
      {
        if(data > (uint8_t *)memHeap + heapSize)
        {
