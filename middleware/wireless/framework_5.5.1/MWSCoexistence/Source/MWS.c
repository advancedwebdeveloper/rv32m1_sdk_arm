...
{ 
  if(rf_deny)
  {
    ...
    case gMWS_PinInterruptFallingEdge_c:
      rf_deny->interruptSelect = pinInt_FallingEdge_c;
      break;
    
    case gMWS_PinInterruptRisingEdge_c:
      rf_deny->interruptSelect = pinInt_RisingEdge_c;
      break;
    
    case gMWS_PinInterruptEitherEdge_c:                  
      rf_deny->interruptSelect = pinInt_EitherEdge_c;
      break;
      
    default:
      rf_deny->interruptSelect = pinInt_Invalid_c;      
      break;
    }
...
static void MWS_DisableRfDenyPinInterrupt(void)
{
  if(rf_deny)
  {        
    rf_deny->interruptSelect = pinInt_Disabled_c;    
    GpioSetPinInterrupt(rf_deny->gpioPort, rf_deny->gpioPin, rf_deny->interruptSelect);
  }
}
#endif
