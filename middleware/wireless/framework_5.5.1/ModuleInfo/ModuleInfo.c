...
*
********************************************************************************** */
#if defined(CPU_RV32M1_cm0plus) && (CPU_RV32M1_cm0plus == 1)
...
uint8_t ModVer_GetInfoFSCIFormat_Multicore(uint8_t* pString)
{
    ...
    {
        if((totalLen + gVERSION_TAGS_entrySizeNoPadding_d -
           GetSizeOfMember(moduleInfo_t, moduleString)) > MAX_REGISTERED_MODULES_STRLEN)
        {
            break;
        }
        ...
        totalLen += gVERSION_TAGS_entrySizeNoPadding_d - GetSizeOfMember(moduleInfo_t, moduleString);
        pInfo++;
    }
...
#endif /* defined(CPU_RV32M1_cm0plus) && (CPU_RV32M1_cm0plus == 1) */