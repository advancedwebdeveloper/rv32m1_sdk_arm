...
static gpioStatus_t Gpio_InstallPortISR(IRQn_Type irqId, uint32_t nvicPrio)
{
    if( irqId != NotAvail_IRQn )
    {
       ...
       {
           INTMUX_Init(INTMUX0);
           INTMUX_EnableInterrupt(INTMUX0, irqId / 32, irqId);
       }
#endif
    }
    return gpio_success;
...
void Gpio_IsrHandle(uint8_t irqNumber)
{
	uint32_t flags;
    /* Search for the highest priority ISR installed for the current PORT */
    for(int i=0; i<mGpioIsrCount; i++ )
    {
#if (FSL_FEATURE_SOC_INTMUX_COUNT <= 0) && (FSL_FEATURE_SOC_SYSCON_COUNT > 0)
        flags = GPIO_GetPinsInterruptFlags(maPortBases[mGpioIsr[i].port]);
#else
...
{
	Gpio_IsrHandle(maPortIrqId[0]);
}
void PORTB_IRQHandler()
{
	Gpio_IsrHandle(maPortIrqId[1]);
}
void PORTC_IRQHandler()
{
	Gpio_IsrHandle(maPortIrqId[2]);
}
void PORTD_IRQHandler()
{
	Gpio_IsrHandle(maPortIrqId[3]);
}
void PORTE_IRQHandler()
{
	Gpio_IsrHandle(maPortIrqId[4]);
}
