...
#if RADIO_IS_GEN_3P0
#define RDINDX                  (0x41U)
#define K3_BASE_INDEX           (0x11U) /* Base for read index */
#else
#define RDRSRC                  (0x03U)
#if RADIO_IS_GEN_2P1
#define FTFA    (FTFE)
#define KW3x_BASE               (0x58)
#else
 ...
 * The only difference in API is return value size changes to 64 bit for Gen 2.1.
 ***********************************************************************************/
#if RADIO_IS_GEN_2P1
...
uint32_t read_another_ifr_word(void)
{
    ...
    if (num_words_avail == 0)
    {
        ...
        ifr_read_addr+=8; /* Read resource addresses increment by 8 due to 64 bit reads */
#endif /* RADIO_IS_GEN_3P0 */
...
uint64_t read_index_ifr(uint32_t read_addr)
{
    ...
    if ((FTFE_FSTAT_ACCERR_MASK & FTFE->FSTAT)!=0)
    {
      //  __asm("bkpt 0xFF");  /* assembly breakpoint */
    }
    OSA_InterruptEnable();
...
#if RADIO_IS_GEN_2P1
/*! *********************************************************************************
 * \brief  Process block 0 IFR data.
 *
 * \param sw_trim_tbl not used, left to maintain driver compatibility
 * \param num_entries not used, left to maintain driver compatibility
 *
 * \remarks
 *  Since there is no autoload feature for KW35Ax/KW36x devices, this routine
 *  restores the trims in the IFR0 instead of using the previous IFR table processing
 *
 ***********************************************************************************/
void handle_ifr(IFR_SW_TRIM_TBL_ENTRY_T * sw_trim_tbl, uint16_t num_entries)
{
    ...
    // Read value from UIFR BLOCK_0
    packed_data = read_resource_ifr(KW3x_BASE);
    dest_data = ((packed_data >> 24) & 0xFF);

    if (dest_data == 0xFF)
    {
        /* Valid data is not present, use blind IFR trim. */
        RSIM->ANA_TRIM &= ~(RSIM_ANA_TRIM_BG_1V_TRIM_MASK |
                            RSIM_ANA_TRIM_BG_IBIAS_5U_TRIM_MASK);
        // Update the content of ANA_TRIM
        RSIM->ANA_TRIM |= (RSIM_ANA_TRIM_BG_1V_TRIM(0x8) |
                           RSIM_ANA_TRIM_BG_IBIAS_5U_TRIM(0x7));
    }
    else
    {
        /* Valid data was found, process real IFR data */
        RSIM->ANA_TRIM &= ~(RSIM_ANA_TRIM_BG_1V_TRIM_MASK |
                            RSIM_ANA_TRIM_BG_IBIAS_5U_TRIM_MASK);
        // Update the content of ANA-TRIM
        RSIM->ANA_TRIM |= (RSIM_ANA_TRIM_BG_1V_TRIM(dest_data & 0xF) |
                           RSIM_ANA_TRIM_BG_IBIAS_5U_TRIM(dest_data >> 4));
    }
}
#else
/*! *********************************************************************************
