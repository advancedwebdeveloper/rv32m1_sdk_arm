...
A_STATUS WIFISHIELD_InitDrivers(void *param)
{
...
#if defined(WIFISHIELD_DMA) && defined(WIFISHIELD_DMAMUX) && 0
    /* configure dma_mode */
    ...
    gpio_pin_config_t config;
    memset(&config, 0, sizeof(config));
    static const IRQn_Type intmuxIrqs[][FSL_FEATURE_INTMUX_CHANNEL_COUNT] = INTMUX_IRQS;
    ...
    CLOCK_EnableClock(WIFISHIELD_WLAN_IRQ_INTMUX_CLOCKSRC);
    INTMUX_EnableInterrupt(WIFISHIELD_WLAN_IRQ_INTMUX, WIFISHIELD_WLAN_IRQ_INTMUX_CHNL, WIFISHIELD_WLAN_IRQn);

    /* Enable NVIC interrupt for WLAN_IRQ */
    EnableIRQ(intmuxIrqs[WIFISHIELD_WLAN_IRQ_INTMUX_INSTANCE][WIFISHIELD_WLAN_IRQ_INTMUX_CHNL]);

    /* Set NVIC priority if is required by Freertos */
    NVIC_SetPriority(intmuxIrqs[WIFISHIELD_WLAN_IRQ_INTMUX_INSTANCE][WIFISHIELD_WLAN_IRQ_INTMUX_CHNL], WIFISHIELD_WLAN_IRQ_PRIORITY);
#endif
...
A_STATUS WIFISHIELD_PowerUp(uint32_t enable)
{
    if (enable)
    {
        GPIO_SetPinsOutput(WIFISHIELD_WLAN_PWRON_GPIO, (1 << (WIFISHIELD_WLAN_PWRON_PIN)));
    }
    else
    {
        GPIO_ClearPinsOutput(WIFISHIELD_WLAN_PWRON_GPIO, (1 << (WIFISHIELD_WLAN_PWRON_PIN)));
    }
    return A_OK;
}
...
{
    uint32_t pin_flags = GPIO_GetPinsInterruptFlags(WIFISHIELD_WLAN_IRQ_GPIO);
    GPIO_ClearPinsInterruptFlags(WIFISHIELD_WLAN_IRQ_GPIO, pin_flags);
